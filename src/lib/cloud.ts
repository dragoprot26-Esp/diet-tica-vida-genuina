 ...
 *
 * cloud.ts — Capa de nube (Supabase) para Dietética Vida Genuina.
...
export async function asegurarCuentaSeguraDueno(usuario: string, password: string, codigo: string) {
  ...
  // Sincroniza la clave del dueño con la licencia (texto plano en panel) y vincula membresía.
...
export async function asegurarCuentaSeguraColab(usuario: string, password: string, codigo: string) {
  ...
  if (!sess) {
    let ok = false;
    try { ok = await rpc('vida_verificar_colab', { p_codigo: codigo, p_usuario: usuario, p_pass: password }, false); }
    catch (e) { ok = false; }
  ...
  try { await rpc('vida_unir_colab', { p_codigo: codigo, p_usuario: usuario }); }
...
// ── Sync de datos del local (tabla vida_backups, RLS por membresía) ────
export async function cloudLoad(codigo: string): Promise<CloudData | null> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(
      `${SB_URL}/rest/v1/vida_backups?tenant_id=eq.${encodeURIComponent(codigo)}&select=datos&limit=1`,
...
export async function cloudSave(codigo: string, datos: CloudData): Promise<boolean> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(`${SB_URL}/rest/v1/vida_backups`, {
      method: 'POST',
      headers: {
...
export async function vidaPublica(codigo: string): Promise<CloudData | null> {
  try { return await rpc('vida_publica', { p_codigo: codigo }, false) as CloudData; }
  catch (e) { return null; }
}

export async function vidaAgregarPedido(codigo: string, pedido: any): Promise<void> {
